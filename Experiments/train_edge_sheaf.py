"""
Training script for Edge Sheaf Laplacian Network on MUTAG dataset
...
from models.edge_sheaf_laplacian import EdgeSheafLaplacianNetwork
...
def main():
    # Hyperparameters
    stalk_dim = 4  # Dimension of stalks attached to each edge
    hidden_dims = [32, 64]
    num_classes = 2
    sheaf_type = 'orthogonal'  # Options: 'diagonal', 'orthogonal', 'general'
    mlp_hidden_dim = 128
    dropout = 0.5
    ...
    # Model
    print(f"\n=== Initializing Edge Sheaf Laplacian Network (sheaf_type={sheaf_type}) ===")
    print("Hybrid approach: Hodge Laplacian + Sheaf Neural Network")
    model = EdgeSheafLaplacianNetwork(
    ...
    # Training loop
    print("\n=== Training Edge Sheaf Laplacian Network ===")
    best_test_acc = 0.0
    best_epoch = 0
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        if test_acc > best_test_acc:
            ...
            best_epoch = epoch
            # Save best model
            torch.save(model.state_dict(), f'best_edge_sheaf_{sheaf_type}.pt')

    # Final evaluation
    print("\n=== Final Evaluation ===")
    model.load_state_dict(torch.load(f'best_edge_sheaf_{sheaf_type}.pt'))
    ...
    # Display learned balance parameters
    print("\n=== Learned Balance Parameters ===")
    for i, layer in enumerate(model.diffusion_layers):
        alpha = torch.sigmoid(layer.alpha).item()
        beta = torch.sigmoid(layer.beta).item()
        print(f"Layer {i+1}: α (Hodge weight) = {alpha:.4f}, β (Sheaf weight) = {beta:.4f}")

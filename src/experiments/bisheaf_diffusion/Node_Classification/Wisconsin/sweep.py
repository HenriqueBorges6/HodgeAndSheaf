"""
Grid search + K-Fold CV para BiSheafNodeClassifier no dataset Wisconsin.

Auto-contido. Edite este arquivo diretamente para ajustar o pipeline.

Uso:
    # Grid completo (48 configs x 10 splits = 480 jobs)
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Wisconsin.sweep

    # Smoke test (1 split por config = 48 jobs)
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Wisconsin.sweep --n-splits 1

    # Limite manual de workers paralelos
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Wisconsin.sweep --workers 4

    # Forçar 1 worker (mostra tqdm de epocas)
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Wisconsin.sweep --workers 1

    # Ver jobs sem rodar
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Wisconsin.sweep --dry-run

    # Retomar run parcial
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Wisconsin.sweep --resume results/grid_partial.json
"""
...
def _print_ranking(aggregated: dict, top_n: int = 10) -> None:
    ...
    sep = "=" * 68
    print(f"\n{sep}")
    print(f"  Top {min(top_n, len(entries))} configs — Wisconsin / test acc (mean +- std %)")
    print(f"{sep}")
...
def _print_efficiency_report(telemetry: Telemetry, total_time_s: float,
    ...
    sep = "=" * 50
    print(f"\n{sep}")
    print(f"  Relatorio de Eficiencia — Wisconsin Grid Search")
    print(f"{sep}")
...
def _dry_run(jobs: list[dict]) -> None:
    ...
    print(f"\n{'='*60}")
    print(f"  DRY RUN — Wisconsin Grid Search")
    print(f"{'='*60}\n")
...
def main(
    ...
    print(f"\n{'='*60}")
    print(f"  BiSheafDiffusion — Wisconsin Grid Search")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Wisconsin Grid Search — BiSheafNodeClassifier")

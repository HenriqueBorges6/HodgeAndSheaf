"""
Grid search para BiSheafNodeClassifier no dataset Citeseer.

Citeseer é Planetoid (split único), portanto n_seeds por config.

Uso:
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Citeseer.sweep
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Citeseer.sweep --workers 4
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Citeseer.sweep --dry-run
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Citeseer.sweep --resume results/grid/grid_partial.json
"""
...
def _print_ranking(aggregated: dict, top_n: int = 10) -> None:
    ...
    sep = "=" * 68
    print(f"\n{sep}")
    print(f"  Top {min(top_n, len(entries))} configs — Citeseer / test acc (mean +- std %)")
    print(f"{sep}")
...
def _print_efficiency_report(telemetry: Telemetry, total_time_s: float,
    ...
    sep = "=" * 50
    print(f"\n{sep}")
    print(f"  Relatorio de Eficiencia — Citeseer Grid Search")
    print(f"{sep}")
...
def _dry_run(jobs: list[dict], n_seeds: int) -> None:
    ...
    print(f"\n{'='*60}")
    print(f"  DRY RUN — Citeseer Grid Search")
    print(f"{'='*60}\n")
...
def main(
    ...
    print(f"\n{'='*60}")
    print(f"  BiSheafDiffusion — Citeseer Grid Search")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Citeseer Grid Search — BiSheafNodeClassifier")

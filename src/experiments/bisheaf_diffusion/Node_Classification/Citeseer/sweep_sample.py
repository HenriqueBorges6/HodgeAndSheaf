"""
Random search sobre o grid Citeseer — amostra N configs aleatórias.

Citeseer é Planetoid (split único), portanto n_seeds por config.
...
Uso:
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Citeseer.sweep_sample
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Citeseer.sweep_sample --n-samples 500
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Citeseer.sweep_sample --n-samples 200 --sample-seed 42
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Citeseer.sweep_sample --resume results/grid/sample_partial.json
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Citeseer.sweep_sample --dry-run
"""
...
from src.experiments.bisheaf_diffusion.Node_Classification.Citeseer.config import BASE_CONFIG, GRID
from src.experiments.bisheaf_diffusion.Node_Classification.Citeseer.sweep import (
    _TRAIN_KEYS,
    _LOCK,
    Telemetry,
    _decide_workers,
...
def _build_jobs_sample(
    ...
    jobs = []
    for cfg_name, params in _generate_sample(n_samples, sample_seed):
...
def _save_partial(raw_results: list[dict]) -> None:
    ...
    with open(tmp, "w") as f:
        json.dump({"dataset": "Citeseer", "raw": raw_results}, f, indent=2)
    os.replace(tmp, path)
...
def _save_final(raw_results: list[dict], aggregated: dict, telemetry: Telemetry) -> None:
    ...
    with open(os.path.join(_RESULTS_DIR, "sample_results.json"), "w") as f:
        json.dump({"dataset": "Citeseer", "raw": raw_results, "aggregated": aggregated}, f, indent=2)
...
def main(
    ...
    jobs = _build_jobs_sample(n_samples, n_seeds, sample_seed, show_progress, skip_done)

    total_grid  = len(_full_grid_list())
    sample_size = min(n_samples, total_grid)

    if dry_run:
        sample = _generate_sample(n_samples, sample_seed)
        print(f"\n{'='*60}")
        print(f"  DRY RUN — Citeseer Random Search")
        print(f"{'='*60}\n")
        print(f"  Grid total      : {total_grid:,} configs")
        print(f"  Amostra         : {sample_size} configs  (sample_seed={sample_seed})")
    ...
    print(f"\n{'='*60}")
    print(f"  BiSheafDiffusion — Citeseer Random Search")
    print(f"  Grid total      : {total_grid:,} configs")
    print(f"  Amostra         : {sample_size} configs  (sample_seed={sample_seed})")
    print(f"  Seeds por config: {n_seeds}  (variância de otimização)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Citeseer Random Search — BiSheafNodeClassifier")
    ...
    parser.add_argument("--sample-seed",     type=int,   default=0,
                        help="Semente para reprodutibilidade da amostragem do grid (default 0).")
    ...
    parser.add_argument("--save-every",      type=int,   default=10,
                        help="Salva sample_partial.json a cada N jobs (default 10).")

"""
Grid search + K-Fold CV para BiSheafNodeClassifier no dataset Texas.

Auto-contido. Edite este arquivo diretamente para ajustar o pipeline.

Uso:
    # Grid completo (48 configs x 10 splits = 480 jobs)
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Texas.sweep

    # Smoke test (1 split por config = 48 jobs)
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Texas.sweep --n-splits 1

    # Limite manual de workers paralelos
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Texas.sweep --workers 4

    # Forçar 1 worker (mostra tqdm de epocas)
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Texas.sweep --workers 1

    # Ver jobs sem rodar
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Texas.sweep --dry-run

    # Retomar run parcial
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Texas.sweep --resume results/grid_partial.json
"""
...
from src.experiments.bisheaf_diffusion.Node_Classification.Texas.config import BASE_CONFIG, GRID
...
def _print_ranking(aggregated: dict, top_n: int = 10) -> None:
    ...
    sep = "=" * 68
    print(f"\n{sep}")
    print(f"  Top {min(top_n, len(entries))} configs — Texas / test acc (mean +- std %)")
    print(f"{sep}")
...
def _print_efficiency_report(telemetry: Telemetry, total_time_s: float,
    ...
    sep = "=" * 50
    print(f"\n{sep}")
    print(f"  Relatorio de Eficiencia — Texas Grid Search")
    print(f"{sep}")
...
def _save_partial(raw_results: list[dict]) -> None:
    ...
    with open(tmp, "w") as f:
        json.dump({"dataset": "Texas", "raw": raw_results}, f, indent=2)
    os.replace(tmp, path)
...
def _save_final(raw_results: list[dict], aggregated: dict, telemetry: Telemetry) -> None:
    ...
    with open(os.path.join(_RESULTS_DIR, "grid_results.json"), "w") as f:
        json.dump({"dataset": "Texas", "raw": raw_results, "aggregated": aggregated}, f, indent=2)
...
def _dry_run(jobs: list[dict]) -> None:
    ...
    print(f"\n{'='*60}")
    print(f"  DRY RUN — Texas Grid Search")
    print(f"{'='*60}\n")
...
def main(
    ...
    print(f"\n{'='*60}")
    print(f"  BiSheafDiffusion — Texas Grid Search")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Texas Grid Search — BiSheafNodeClassifier")

"""
Random search sobre o grid Cora — amostra N configs aleatórias.
...
Uso:
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cora.sweep_sample
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cora.sweep_sample --n-samples 500
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cora.sweep_sample --n-samples 200 --seed 42
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cora.sweep_sample --resume results/grid/sample_partial.json
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cora.sweep_sample --dry-run
"""
...
from src.experiments.bisheaf_diffusion.Node_Classification.Cora.sweep import (
    _TRAIN_KEYS,
    _LOCK,
    Telemetry,
    _decide_workers,
...
def main(
    ...
    if dry_run:
        ...
        print(f"\n{'='*60}")
        print(f"  DRY RUN — Cora Random Search")
        print(f"{'='*60}\n")
    ...
    print(f"\n{'='*60}")
    print(f"  BiSheafDiffusion — Cora Random Search")
    print(f"  Grid total      : {total_grid:,} configs")
    print(f"  Amostra         : {sample_size} configs  (sample_seed={sample_seed})")
    print(f"  Seeds por config: {n_seeds}  (variância de otimização)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Cora Random Search — BiSheafNodeClassifier")

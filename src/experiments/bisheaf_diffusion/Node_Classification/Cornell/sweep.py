"""
Grid search + K-Fold CV para BiSheafNodeClassifier no dataset Cornell.

Auto-contido. Edite este arquivo diretamente para ajustar o pipeline.

Uso:
    # Grid completo (48 configs x 10 splits = 480 jobs)
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cornell.sweep

    # Smoke test (1 split por config = 48 jobs)
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cornell.sweep --n-splits 1

    # Limite manual de workers paralelos
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cornell.sweep --workers 4

    # Forçar 1 worker (mostra tqdm de epocas)
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cornell.sweep --workers 1

    # Ver jobs sem rodar
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cornell.sweep --dry-run

    # Retomar run parcial
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cornell.sweep --resume results/grid_partial.json
"""
...
# Heartbeat — quanto tempo entre prints "ainda vivo" do main thread.
_HEARTBEAT_INTERVAL_S = 30

from src.experiments.bisheaf_diffusion.Node_Classification.Cornell.config import BASE_CONFIG, GRID
...
def _gpu_query() -> tuple[float, float]:
    """(free_mb, util_pct) via nvidia-smi. Retorna (inf, 0) se indisponivel."""
    try:
        r = subprocess.run(
...
def _ram_query() -> tuple[float, float]:
    """(free_mb, used_pct) via psutil. Retorna (inf, 0) se indisponivel."""
    try:
        import psutil
...
class Telemetry:
    """
    Coleta amostras de GPU/RAM em thread daemon a cada `interval_s` segundos.
    O scheduler NUNCA chama nvidia-smi — apenas le `samples` quando relevante.
    """
...
# Cache de dataset por processo. Cada worker carrega 1x e reusa em todos
# os jobs subsequentes do mesmo dataset (workers sao reusados pelo Pool).
_WORKER_CACHE: dict = {}


def _run_job(job: dict) -> dict:
    """Roda um (config, split). Cache de dataset por worker."""
    import gc
    import os
    import warnings
    warnings.filterwarnings("ignore")

    # Deve ser antes do import do torch para afetar OpenMP/MKL na inicialização.
...
def _make_config_name(params: dict) -> str:
    ...
    mF  = params["map_type_F"][:3]   # gen | ort | dia
...
def _build_jobs(n_splits: int, show_progress: bool, skip_done: set = None) -> list[dict]:
...
def _decide_workers(
    ...
    """
    Decide n_workers a partir de VRAM livre, RAM livre e cap manual.
    Retorna (n_workers, info_dict) para logging.
    """
...
def _run_pool(jobs: list[dict], n_workers: int, on_result, telemetry: Telemetry) -> list[dict]:
    ...
    Executa todos os jobs com submissao em fluxo:
      - mantem no maximo `n_workers * 2` futures em voo (evita criar 480
        objetos de future de uma vez na memoria do processo principal)
    ...
    Workers ainda sao reusados para preservar o cache de dataset.
    """
    ...
    submit_time: dict   = {}  # future -> timestamp em que foi submetido
    last_completion = [time.time()]  # mutavel, em lista pra fechar no thread
    ...
    # Heartbeat thread — imprime status do pool a cada _HEARTBEAT_INTERVAL_S
    ...
    def _heartbeat_loop():
        while not hb_stop.is_set():
            ...
            ages   = [int(now - submit_time.get(f, now)) for f in list(in_flight)]
    ...
    def _force_kill_workers(executor: ProcessPoolExecutor) -> None:
        """Mata processos worker do executor. cancel_futures so cancela o que
        ainda nao comecou — workers em deadlock continuam rodando. Isso aqui
        garante que eles morram pra liberar RAM/GPU."""
...
def _print_ranking(aggregated: dict, top_n: int = 10) -> None:
    ...
    sep = "=" * 68
    print(f"\n{sep}")
    print(f"  Top {min(top_n, len(entries))} configs — Cornell / test acc (mean +- std %)")
    print(f"{sep}")
...
def _print_efficiency_report(telemetry: Telemetry, total_time_s: float,
    ...
    sep = "=" * 50
    print(f"\n{sep}")
    print(f"  Relatorio de Eficiencia — Cornell Grid Search")
    print(f"{sep}")
...
def _save_partial(raw_results: list[dict]) -> None:
    ...
    with open(tmp, "w") as f:
        json.dump({"dataset": "Cornell", "raw": raw_results}, f, indent=2)
    os.replace(tmp, path)
...
def _save_final(raw_results: list[dict], aggregated: dict, telemetry: Telemetry) -> None:
    ...
    with open(os.path.join(_RESULTS_DIR, "grid_results.json"), "w") as f:
        json.dump({"dataset": "Cornell", "raw": raw_results, "aggregated": aggregated}, f, indent=2)
...
def _dry_run(jobs: list[dict]) -> None:
    grid_pts = _generate_grid()
    n_splits = max((j["split_idx"] for j in jobs), default=0) + 1
    print(f"\n{'='*60}")
    print(f"  DRY RUN — Cornell Grid Search")
    print(f"{'='*60}\n")
...
def main(
    ...
    jobs = _build_jobs(n_splits, show_progress, skip_done)
    ...
    print(f"\n{'='*60}")
    print(f"  BiSheafDiffusion — Cornell Grid Search")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Cornell Grid Search — BiSheafNodeClassifier")

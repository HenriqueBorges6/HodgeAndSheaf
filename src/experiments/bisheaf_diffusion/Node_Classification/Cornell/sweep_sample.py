"""
Random search sobre o grid Cornell — amostra N configs aleatórias.
...
Uso:
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cornell.sweep_sample
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cornell.sweep_sample --n-samples 500
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cornell.sweep_sample --n-samples 200 --seed 42
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cornell.sweep_sample --resume results/grid/sample_partial.json
    python -m src.experiments.bisheaf_diffusion.Node_Classification.Cornell.sweep_sample --dry-run
"""
...
from src.experiments.bisheaf_diffusion.Node_Classification.Cornell.sweep import (
    _TRAIN_KEYS,
    Telemetry,
    _decide_workers,
    _make_config_name,
...
def main(
    ...
    if dry_run:
        ...
        print(f"\n{'='*60}")
        print(f"  DRY RUN — Cornell Random Search")
        print(f"{'='*60}\n")
    ...
    print(f"\n{'='*60}")
    print(f"  BiSheafDiffusion — Cornell Random Search")
    print(f"  Grid total      : {total_grid:,} configs")
    print(f"  Amostra         : {sample_size} configs  (seed={seed})")
    print(f"  Splits          : {n_splits}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Cornell Random Search — BiSheafNodeClassifier")
